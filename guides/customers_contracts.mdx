...
- call [create customer link](api-reference/customers/create_link) to receive an `authorizationUrl` link.
...
- When the user approves, the web view will be redirected to the webpage defined by the `successUrl` parameter in the [create customer link](api-reference/customers/create_link) call, and you will be able to access the users data via the API.
...
1. You can add a parameter `externalId` to the [create customer link](api-reference/customers/create_link) call. This information is then stored with the customer data in the RABOT platform, and returned as part of the customer data in the [list customers](api-reference/customers/list_customers) endpoint.
...
> Your application needs 
 `role:application`, `role:customer-support` or `role: finance-reporting` scope to access contract information.
...
> Your application needs 
 `role:application` scope to access contract metrics.
